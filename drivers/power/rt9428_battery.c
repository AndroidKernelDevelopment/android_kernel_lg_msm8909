...
#if defined(CONFIG_LGE_PM_BATTERY_RT9428_SC_ALERT_RECOVER)
	// richtek: add the patch below to check fg Alert pin ; 201605027

	status_val = rt9428_reg_read_word(chip->i2c, RT9428_REG_STATUS);
	config_val = rt9428_reg_read_word(chip->i2c, RT9428_REG_CFG0);

	// if
	//	  (1)[SC]@Reg-Status is set OR
	//	  (2)[Alert]@Reg-Config is set
	// then
	//	  (0)AP check GPIO status and print debug message
	//	  (1)clear [SC] anyway
	//	  (2)clear [ALERT] anyway then set [SCEN] bit
	//	  (3)Do "rt9428_update_info" for updat SOC

	if((status_val&0x2000)||((config_val&0x0020))){
		//(0)AP check GPIO status and print debug message
		//AP get Alert pin GPIO status and print debug message
