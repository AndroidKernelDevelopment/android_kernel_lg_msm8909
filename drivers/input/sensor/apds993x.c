...
static int apds9930_backup_crosstalk_data_fs(unsigned int val)
{
	int fd;
	int ret = 0;
	char buf[50];
...
static int apds9930_read_crosstalk_data_fs(void)
{
	int fd;
	int ret = 0;
	int len = 0;
...
//		data->ps_threshold - SUB_FROM_PS_THRESHOLD;
	...
	apds9930_backup_crosstalk_data_fs(data->cross_talk);
		...
		struct i2c_client *client, int cal_data)
{
	...
	pr_info("%s: adjusted threshold, near_offset=%d, far_offset=%d\n", __func__, data->ps_threshold, data->ps_hysteresis_threshold);
}
...
static void apds993x_change_ps_threshold(struct i2c_client *client)
{
	...
	if ((data->ps_data > data->pilt) && (data->ps_data >= data->piht)) {
		...
		data->piht = 1023;
		pr_info("%s: FAR-to-NEAR, pilt:%d, piht:%d, pdata:%d\n", __func__, data->ps_hysteresis_threshold, 1023, data->ps_data);
		...
		pr_info("%s: NEAR-to-FAR, pilt:%d, piht:%d, pdata:%d\n", __func__, 0, data->ps_threshold, data->ps_data);
	}
}
...
static void apds993x_work_handler(struct work_struct *work)
{
	...
	if ((status & 0x40) == 0x40){
		pr_info("%s, %d: psat saturation >> near-to-far\n", __func__, __LINE__);
		//if (data->ps_detection == 1) {
		pr_info("apds >>>>: [piht][pilt][c_t] = [%d][%d][%d]\n",
				data->ps_threshold,
				data->ps_hysteresis_threshold,
				data->cross_talk);
		/* near-to-far detected */
		...
		if (ch0data <
			...
			if (data->ps_detection == 1)
				...
				pr_info("%s: background ambient noise, ch0data=%d\n", __func__, ch0data);
		}
...
static int apds993x_enable_als_sensor(struct i2c_client *client, int val)
{
	...
	pr_info("%s: enable_als_sensor finished\n", __func__);
		...
		unsigned int val)
{
	...
	if (ret >= 0) {
		...
		pr_info("poll delay %d, atime_index %d\n",
...
static int apds993x_enable_ps_sensor(struct i2c_client *client, int val)
{
	...
	if (val == 1) {
		...
		if (data->enable_ps_sensor == 0) {
			...
			data->cross_talk = apds9930_read_crosstalk_data_fs();
...
//	struct i2c_client *client = to_i2c_client(dev);
	...
	pr_info("%s: nSecval=%d, val=%d", __func__, (int)nSecval, (int)val);
		...
		struct apds993x_platform_data *pdata)
{
	...
	dev_info(dev, "%s: ps-threshold = %d\n", __func__, pdata->prox_threshold);
	...
	dev_info(dev, "%s: ps-hysteresis-threshold = %d\n", __func__, pdata->prox_hysteresis_threshold);
	...
	dev_info(dev, "%s: cross-talk = %d\n", __func__, pdata->cross_talk);
	...
	dev_info(dev, "%s: ps-pulse = %d\n", __func__, pdata->prox_pulse);
	...
	pdata->als_B = tmp;
	dev_info(dev, "%s: als-B = %d\n", __func__, pdata->als_B);
	...
	pdata->als_C = tmp;
	dev_info(dev, "%s: als-C = %d\n", __func__, pdata->als_C);
	...
	pdata->als_D = tmp;
	dev_info(dev, "%s: als-D = %d\n", __func__, pdata->als_D);
	...
	pdata->ga_value = tmp;
	dev_info(dev, "%s: ga-value = %d\n", __func__, pdata->ga_value);

	pdata->default_cal = of_property_read_bool(np, "avago,default-cal");
	dev_info(dev, "%s: default-cal = %d\n", __func__, pdata->default_cal);
