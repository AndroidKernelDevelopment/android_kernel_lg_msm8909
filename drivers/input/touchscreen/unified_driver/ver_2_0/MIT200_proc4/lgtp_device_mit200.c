...
int mip_lpwg_config(struct i2c_client* client)
{
	...
	wbuf[9] = 480 & 0xFF;					// LPWG_ACTIVE_AREA (horizontal end low byte)
	wbuf[10] = 480 >> 8 & 0xFF;				// LPWG_ACTIVE_AREA (horizontal end high byte)
	wbuf[11] = 854 & 0xFF; 				// LPWG_ACTIVE_AREA (vertical end low byte)
	wbuf[12] = 854 >> 8 & 0xFF;			// LPWG_ACTIVE_AREA (vertical end high byte)	
	wbuf[13] = 0;							// LPWG_FAIL_REASON
...
int mip_lpwg_config_knock_on(struct i2c_client* client)
{
	...
	wbuf[14] = (DELAY_ENABLE ? KNOCKON_DELAY : 700) & 0xFF;		// LPWG_INTERTAP_DELAY (low byte)
	wbuf[15] = ((DELAY_ENABLE ? KNOCKON_DELAY : 700) >> 8) & 0xFF;	// LPWG_INTERTAP_DELAY (high byte)
