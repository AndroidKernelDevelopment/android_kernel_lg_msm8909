...
static char defaultFirmware[] = "synaptics/k6p/PLG622-V0.03_PR2386136_DS5.12.3.3.1008_20053103.img";
static char defaultFirmwareRecovery[] = "synaptics/k6p/PLG622-V0.03_PR2386136_DS5.12.3.3.1008_20053103.bin";
...
#if !defined (TOUCH_MODEL_K6P)// || !defined(TOUCH_MODEL_K6M)
	switch (value) {
	case IME_OFF:
		if (ime_stat) {
			ime_stat = 0;
...
static void TD4100_SetCoverStatus(void)
{
	...
	if(QCoverClose){
		TOUCH_LOG( "%s : Cover Closed. tTouchWriteByteReg(0x1c, 0x02) \n" , __func__);
		TouchWriteByteReg(0x1c, 0x02);
	} else {
		TOUCH_LOG( "%s : Cover Open. tTouchWriteByteReg(0x1c, 0x00) \n" , __func__);
		TouchWriteByteReg(0x1c, 0x00);
	}
...
//TOUCH_FUNC();
  ...
  DO_IF((regDevStatus & DEVICE_FAILURE_MASK)== DEVICE_FAILURE_MASK, error);
	...
	if (regIntStatus == INTERRUPT_MASK_NONE) {
		...
		TOUCH_LOG("4100 irq handler 9");
		TOUCH_ERR( "Unexpected Interrupt Status ( 0x%X )\n", regIntStatus );
	}

	return TOUCH_SUCCESS;
error:
	TOUCH_LOG("4100 irq handler 10");
